...
const PROMPT_DESCRIPTIONS = {
    ...
    'actionSuggestionsPrompt': 'Prompt for generating action suggestions (max 3 options)'
};
...
export function renderPromptEditorSection() {
    // Merge saved prompts with defaults to ensure all prompts are available
    const prompts = { ...DEFAULT_PROMPTS };

    // Override with saved values if they exist
    if (extensionSettings.prompts) {
        for (const [key, value] of Object.entries(extensionSettings.prompts)) {
            if (typeof value === 'object' && value !== null && typeof prompts[key] === 'object') {
                prompts[key] = { ...prompts[key], ...value };
